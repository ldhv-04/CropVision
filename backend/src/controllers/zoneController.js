...
const publishZones = async (req, res) => {
  ...
  try {
    ...
    // Keep non-mutating validation before BEGIN to preserve current 400/404 behavior.
    ...
    const fieldWithOwner = await client.query(
    ...
    if (transactionStarted && client) {
      try {
        ...
        console.error('[Zone] publishZones rollback error:', rollbackError.message);
      }
    }
