...
const mapErrorCopy = {
  NO_PUBLISHED_MAP: {
    ...
    message: 'This field does not have a published map yet.',
  },
  PUBLICATION_CONFLICT: {
    title: 'Publication conflict',
    message: 'The published map is temporarily unavailable. Refresh and try again.',
  },
  FIELD_NOT_FOUND: {
...
function MapContractState({ error, onRetry, onBack }) {
  const knownError = mapErrorCopy[error.code];

  return (
    <View style={styles.center}>
      <Text style={styles.errorIcon}>⚠️</Text>
      {knownError ? <Text style={styles.errorTitle}>{knownError.title}</Text> : null}
      <Text style={styles.errorText}>
        {knownError?.message || error.message}
      </Text>
      <TouchableOpacity style={styles.retryBtn} onPress={onRetry}>
        <Text style={styles.retryBtnText}>{knownError ? 'Refresh' : 'Retry'}</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  errorTitle: { fontSize: 20, fontWeight: '700', color: '#424242', marginBottom: 8 },
