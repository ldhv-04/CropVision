...
const useMobileFieldStore = create((set, get) => ({
  ...
  fetchZoneMap: async (fieldId) => {
    ...
    try {
      ...
      let message = 'Could not load zone map.';
      if (status === 403) message = 'You do not have access to this field.';
      else if (status === 404) message = 'No published zone map available for this field.';
