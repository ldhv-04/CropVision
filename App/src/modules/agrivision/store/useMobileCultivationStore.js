...
const useMobileCultivationStore = create((set, get) => ({
  ...
  fetchSummary: async (fieldId, zoneId) => {
    ...
    try {
      ...
      const normalized = normalizeApiError(error, 'Could not load cultivation data.');
      set({
        isLoading: false,
        error: normalized,
  ...
  fetchLogs: async (fieldId, zoneId, page = {}) => {
    if (!fieldId || !zoneId) return;
    set({ isLoadingLogs: true, error: null, conflict: null });
    try {
      ...
      const normalized = normalizeApiError(error, 'Could not load cultivation logs.');
      set({
        isLoadingLogs: false,
        error: normalized,
        conflict: normalized.code === 'PUBLICATION_CONFLICT' ? normalized : null,
  ...
  saveProfile: async (fieldId, zoneId, payload) => {
    ...
    try {
      ...
      const normalized = normalizeApiError(error, 'Could not save profile.');
      set({
        isSaving: false,
        error: normalized,
        conflict: normalized.code === 'PUBLICATION_CONFLICT' ? normalized : null,
  ...
  createLog: async (fieldId, zoneId, payload) => {
    ...
    try {
      ...
      const normalized = normalizeApiError(error, 'Could not create log.');
      set({
        isSaving: false,
        error: normalized,
        conflict: normalized.code === 'PUBLICATION_CONFLICT' ? normalized : null,
  ...
  updateLog: async (fieldId, zoneId, logId, payload) => {
    ...
    try {
      ...
      const normalized = normalizeApiError(error, 'Could not update log.');
      set({
        isSaving: false,
        error: normalized,
        conflict: normalized.code === 'PUBLICATION_CONFLICT' ? normalized : null,
  ...
  deleteLog: async (fieldId, zoneId, logId, publicationVersion) => {
    ...
    try {
      ...
      const normalized = normalizeApiError(error, 'Could not delete log.');
      set({
        isSaving: false,
        error: normalized,
        conflict: normalized.code === 'PUBLICATION_CONFLICT' ? normalized : null,
