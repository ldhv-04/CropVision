...
export function fieldToFeature(field) {
  ...
  try {
    ...
    console.warn('[GEOJSON_WARN] Failed to parse boundary for field:', field.id, e);
    return null;
  }
  ...
  if (geo.type === 'Polygon') {
    ...
    console.warn('[GEOJSON_WARN] Unsupported geometry type:', geo.type, 'for field:', field.id);
    return null;
  }
  ...
  console.log('[GEOJSON_FLOW] feature created', {
    id: feature.id,
...
export function fieldsToFeatureCollection(fields) {
  if (!fields || !Array.isArray(fields)) {
    console.log('[GEOJSON] feature count: 0 (no fields array)');
  ...
  console.log('[GEOJSON_FLOW] converting fields to FeatureCollection', fields.length);
  console.log('[GEOJSON_FLOW] feature count', features.length, 'of', fields.length);
