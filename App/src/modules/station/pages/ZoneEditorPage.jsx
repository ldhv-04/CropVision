...
const S = {
  ...
  publishSummary: { padding: '10px 16px', backgroundColor: '#E8F5E9', borderBottom: '1px solid #C8E6C9', color: '#1B5E20', fontSize: 12 },
  publishSummaryLabel: { fontWeight: '700', marginBottom: 2 },
  publishSummaryLine: { marginRight: 16 },
  loadingContainer: { flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#F8F9FA' },
  loadingText: { color: '#888', marginTop: 12, fontSize: 14 },
};

function buildCommittedPublicationViewModel(publication) {
  ...
  return {
    ...
    mapSummary: publication.version != null && publication.zonesCount != null
      ? `Published map v${publication.version} · ${publication.zonesCount} zone${publication.zonesCount === 1 ? '' : 's'}`
      : null,
...
export function PublishSnapshotFeedback({ publication }) {
  const committed = buildCommittedPublicationViewModel(publication);
  if (!committed) return null;

  return (
    <div style={S.publishSummary} role="status">
      <div style={S.publishSummaryLabel}>Last committed publication</div>
      {committed.mapSummary ? <span style={S.publishSummaryLine}>{committed.mapSummary}</span> : null}
      {committed.publishedLabel ? (
        <span style={S.publishSummaryLine}>
          Committed <time dateTime={committed.publishedAt}>{committed.publishedLabel}</time>
        </span>
      ) : null}
      {committed.status ? <span style={S.publishSummaryLine}>Status: {committed.status}</span> : null}
      {committed.fieldId ? <span style={S.publishSummaryLine}>Field: {committed.fieldId}</span> : null}
    </div>
  );
}
...
export default function ZoneEditorPage({ fieldId, onBack }) {
  const {
    zones, parentField, isLoading, hasUnsavedChanges, lastCommittedPublication, validationResult,
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      const committed = await publishZones();
      const message = buildCommittedPublicationViewModel(committed)?.mapSummary
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        <div style={S.headerRight}>
          ...
          {lastCommittedPublication && <span style={S.publishedBadge}>✓ Snapshot committed</span>}
      ...
      <PublishSnapshotFeedback publication={lastCommittedPublication} />
