...
function UserRow({ user, actionKey, onToggleRole, onDelete }) {
  ...
  const confirmDelete = () => Alert.alert(
    ...
    [
      ...
      { text: 'Xóa', style: 'destructive', onPress: () => onDelete(user.id) },
    ]
  );

  const confirmToggle = () => Alert.alert(
    'Thay đổi quyền',
    `Đổi "${user.email}" thành ${isAdmin ? 'User' : 'Admin'}?`,
    [
      { text: 'Hủy', style: 'cancel' },
      { text: 'Xác nhận', onPress: () => onToggleRole(user) },
    ]
  );
    ...
    <View style={styles.row}>
      ...
      <View style={styles.rowActions}>
        <Badge
          ...
          color={isAdmin ? C.warning : C.info}
        />
        {isActing ? (
...
function SampleRow({ sample, actionKey, onDelete }) {
  ...
  const confirmDelete = () => Alert.alert(
    'Xác nhận xóa',
    `Xóa mẫu "${sample.sample_name}"?`,
    [
      { text: 'Hủy', style: 'cancel' },
      { text: 'Xóa', style: 'destructive', onPress: () => onDelete(sample.id) },
    ]
  );
      ...
      <View style={styles.rowLeft}>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.userEmail}>
            {topDisease?.disease_class?.replace(/_/g, ' ') || 'Không phát hiện bệnh'}
          </Text>
          <Text style={styles.userDate}>
            {sample.owner_email} · {new Date(sample.created_at).toLocaleDateString('vi-VN')}
          </Text>
        </View>
      </View>

      <View style={styles.rowActions}>
        {conf && <Badge label={`${conf}%`} color={confColor} />}
        {isActing ? (
...
export default function StationSystem() {
  ...
  const handleToggleRole = async (user) => {
    try {
      await toggleUserRole(token, user);
  ...
  const handleDeleteSample = async (sampleId) => {
    try {
      await deleteSample(token, sampleId);
    } catch { /* error displayed via store */ }
  };
    ...
    <View testID="station-system" style={styles.root}>
      ...
      <View style={styles.searchWrap}>
        <TextInput
          ...
          style={styles.search}
          placeholder={`Tìm kiếm ${activeTab === 'users' ? 'người dùng' : 'mẫu vật'}...`}
      ...
      {isLoading && users.length === 0 ? (
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => loadAdminData(token, true)} tintColor={C.primary} />}
        >
          {activeTab === 'users' ? (
            ...
            filteredSamples.length === 0 ? (
              <Text style={styles.emptyText}>Không tìm thấy mẫu phân tích</Text>
            ) : (
